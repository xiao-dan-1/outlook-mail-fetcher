...
class StaticUiTests(unittest.TestCase):
    ...
    def test_fetch_scope_defaults_to_selected_account_with_explicit_all_mode(self) -> None:
        ...
        self.assertIn("button.dataset.accountEmail = account.email", js)
        self.assertIn('button.className = `account-row ${isSelected ? "is-selected" : ""}`.trim();', js)
    ...
    def test_account_status_and_logs_use_product_copy_instead_of_raw_debug_terms(self) -> None:
        ...
        self.assertIn("button.title = status ? `${account.email}：${statusLabel}` : account.email", js)
        self.assertIn('button.setAttribute("aria-label", `${account.email}，${isSelected ? "当前选中，" : ""}${accountStatusLabel(status)}`)', js)
